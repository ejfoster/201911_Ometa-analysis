...
df.POM<-subset(df.r, param == "POM" |param == "LF"|param == "C_POM" ); length(unique(df.POM$At)); length(unique(df.POM$ACC))
...
rma.all <- rma.mv(ratio_all, V=SD_all, 
             mods = ~ amend, random= ~1|ACC,
            data=dft, W=w_all, method="ML"); rma.all #only including
