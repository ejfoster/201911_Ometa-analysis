...
```{r, 1. prelimary - load packages, include=FALSE}
...
```{r, 2. load data and subset, response ratio + error calculations, include=FALSE}
...
##########################################################################################################
###
### 5. SUBSET AND CALCULATION SOIL CARBON STOCKS
###
##########################################################################################################
...
=======
>>>>>>> 74a7ff06d46b89f06e47e2ca9ebc3ec56de9db7a
...
<<<<<<< HEAD
```{r, Graphs + Check regression, assumptions, and for correlation among parameters}
##########################################################################################################
###
### Check assumptions of regression 
###
#########################################################################################################
#crop_group + irrigation + crop_system + lat_group +clay_group + pH_group + SOC_int_group + duration_even + C.rate_group + amend_N_kg.ha_group + amend_group + MAP

#1. linearity
plot(df.c$ratio_all, df.c$crop_group) # not even numbers/spread
plot(df.c$ratio_all, df.c$irrigation) # more even numbers/spread
plot(df.c$ratio_all, df.c$crop_system) #even
plot(df.c$ratio_all, df.c$lat) #not linear
plot(df.c$ratio_all, df.c$long) #not linear
plot(df.c$ratio_all, df.c$clay) #not linear
plot(df.c$ratio_all, df.c$pH) #not linear
plot(df.c$ratio_all, df.c$SOC_g.kg) #not linear
plot(df.c$ratio_all, df.c$C.rate_Mg.ha) #not linear
plot(df.c$ratio_all, df.c$C.Nrate) #not linear
plot(df.c$ratio_all, df.c$amend_N_kg.ha) #not linear, 2 outliers
plot(df.c$ratio_all, df.c$MAP)

#2. correlation of covarites (multicolinearity)
=======
```{r, 10. check regression, assumptions, and for correlation among parameters}

>>>>>>> 74a7ff06d46b89f06e47e2ca9ebc3ec56de9db7a
#continuous: C.rate_Mg.ha, C.N_rate, N.amend_g.kg, time.n.mos, Nfert_rate
...
param.corr<-select(df, MAT, MAP, lat.dec.degree, elevation, pH, clay, SOC_g.kg,
                  C.rate_Mg.ha, C.N_rate, amend_N_kg.ha, time.n.mos, Nfert_rate)
param.corr$MAT.MAP<-df$MAT/df$MAP

pear<-rcorr(as.matrix(param.corr), type = "pearson") ; pear
#CORRELATED: pH and MAT = .98 and MAT/MAP =.65; 
           #elevation and time.n.mos (.97); elevation and MAT.MAP .76,  lat and Nfert.rate .333 + amend_N =.32;

<<<<<<< HEAD
### remove elevation and MAT 

# 3. Heteroscadicity (equal variance)

### USING ACC
### GRAPHS OF RESPONSE RATIOS ie, plot within study variance
##########################################################
y<-df.c$ratio_all
...
plot(M, x,ylab="ACC & Treatments", xlab="Effect Size",
 ...
 arrows(y0=x, x0=M-SD, y1=x, x1=M+SD, code=3, angle=90, length=0.025, col="darkgray") #error bars
 arrows(y0=1150, x0=MA-SDA, y1=1150, x1=MA+SDA, code=3, angle=90, length=0.025, lty=2, col="red") #1150 is the 'pretend' At to plot the grand mean
...
#########################################################################################################
#plot within study variance
#########################################################################################################
y<-df.c$ratio_all
x<-as.numeric(levels(df.c$ACC))
   y.f<-df.c$ratio_cf
   y.z<-df.c$ratio_zf
...
plot(M, x,ylab="ACC", xlab="Effect Size",
 ...
 arrows(y0=x, x0=M-SD, y1=x, x1=M+SD, code=3, angle=90, length=0.025, col="darkgray") #error bars
 arrows(y0=1150, x0=MA-SDA, y1=1150, x1=MA+SDA, code=3, angle=90, length=0.025, lty=2, col="blue") #1150 is the 'pretend' ACC to plot the grand mean

points(M, x,pch=16,cex=.75)
points(MA, 1150, col="blue", pch=5,cex=2)
...
```{r, 11. mixed-effect meta-analysis (regression) }
=======
...
```{r, 11. mixed-effect meta-analysis duplicate (regression) }



>>>>>>> 74a7ff06d46b89f06e47e2ca9ebc3ec56de9db7a
length(df.c$ratio_all)
...
#need to get rid of NAs, could impute the data if needed
df.rma<-df.c[which(!is.na(df.c$ratio_all)), ] #delete rows where ratio = NA
<<<<<<< HEAD

#prep data set, remove rows with NA in moderators 
...
>>>>>>> 74a7ff06d46b89f06e47e2ca9ebc3ec56de9db7a
dat <- df.c[!apply(df.c[ , c("crop_group", "irrigation" , "crop_system", "lat_group", "clay_group" ,"pH_group" , "SOC_int_group")], 1, anyNA),]
...
#With level = 1, we stick to models with main effects only 
### ################################################################################# 

rma.glmulti <- function(data, formula, vi)
   rma(data, formula, vi, method="ML")  #important to use ML (instead of REML) estimation, since log-likelihoods (and hence information criteria) are not directly comparable for models with different fixed effects
d<-df.rma
str(df.rma)
res1 <- glmulti(d, ratio_all ~ crop_group + irrigation + crop_system + lat_group +clay_group + pH_group + SOC_int_group, #vi=d$SD_all^2,
               level=1, fitfunction=rma.glmulti, crit="aicc", confsetsize=128)

################################
rma.glmulti <- function(formula, vi, weights, random, data)
   rma(formula, vi, weights, data, random, method="ML", struct="AR")  #important to use ML (instead of REML) estimation, since log-likelihoods (and hence information criteria) are not directly comparable for models with different fixed effects

res1 <- glmulti(df.rma$ratio_all ~ crop_group + irrigation + crop_system + lat_group +clay_group + pH_group + SOC_int_group, vi=df.rma$SD_all, weights=df.rma$w_all, 
                data=df.rma, random=~depth|ACC,
               level=1, fitfunction=rma.glmulti, crit="aicc", confsetsize=128)
################################
...
all.rma.ar.cat <- rma.mv(data=df.c, ratio_all ~ crop_group+irrigation+crop_system+lat_group + clay_group+ pH_group+ SOC_int_group+duration_even+C.rate_group+amend_N_kg.ha_group+amend_group, random= ~depth|ACC, method="REML", V=SD_all, W= w_all, struct="AR") #AIC 60 #BIC 82.38
...
df %>% 
  group_by(ACC, param) %>%
  mutate(rr.zf = log(X_a/X_zf), rr.cf = log(X_a/X_cf)) -> df.grouped
