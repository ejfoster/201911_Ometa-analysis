...
df.c$depth<-(df.c$depth.end+df.c$depth.start)/2 #Courtland - could create depth group here - done!
mean(df.c$depth)
sd(df.c$depth)
max(df.c$depth)

#create subset data frames by depth increment -> categorize based on mean of dpeth increment - note that original df is df2 which has RR's and %change calcs
df.depth.15<-subset(df.c, depth <= 15); nrow(df.depth.15) #subset by depth 0-15
df.depth.30<-subset(df.c, depth <= 30 & depth > 15); nrow(df.depth.30) #subset by depth 15-30cm
df.depth.60<-subset(df.c, depth <= 60 & depth > 30); nrow(df.depth.60) #subset by depth 30-60cm
df.depth.90<-subset(df.c, depth <= 90 & depth > 60); nrow(df.depth.90) #subset by depth 60-90cm

## Some quick stats/analysis with subgroups created
#average RR in each subgroup

mean(na.omit(df.depth.15$ratio_all))#average RR for 0-15 depth
mean(na.omit(df.depth.30$ratio_all))
mean(na.omit(df.depth.60$ratio_all))
mean(na.omit(df.depth.90$ratio_all))

#quick test of subsetted data using model from below
rma1 <- rma.mv(ratio_all, V=SD_all, 
            ...
            data=df.depth.15, W=w_all, method="ML"); rma1
