...
####################################################################################################
###
### 10. Check assumptions of regression  https://peerj.com/articles/3323/
...
rma1 <- rma(ratio_all, vi=SD_all, 
             mods = ~ MAT+ MAP+ lat.dec.degree+ elevation+ pH+ clay+ SOC_g.kg+
                  C.rate_Mg.ha+ C.N_rate+ amend_N_kg.ha+ exp_duration_mos+ Nfert_rate, 
            data=df.c, weights=w_all, method="ML") #important to use ML (instead of REML) estimation, since log-likelihoods (& hence information criteria) are not directly comparable for models with different fixed effects
...
cor.test( na.omit(df.c$clay), res1) #reject null, alternative = true correlation not equal 0
