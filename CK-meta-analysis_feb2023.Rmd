...
##Courtland
Current task:
- new way to calc SD from SE - Erika's code was wrong
-calculate CV's and apply to observations means to impute SD
-use separate CV's for each parameter subset to improve estimate
i.e. cmin_POC has specific CV used to impute SD for that dataset


Current progress
- 798 observations
- calculated lnRR and variance with metafor package function 'escalc'
-subset complete observations by parameter (TOC, MAOC, POC) and control (cmin, cf)
- total complete obs:
MAOC.cmin = 24
POC.cmin = 23
TOC.cmin = 53
...
#from local
df <- read_excel("database_download_3.24.2023.xlsx", sheet = "DATA_3.2.2023", range = "B2:EG1904", na= c("na"))
...
#3) Calculate n-based variance alternative
...
nans <- which(is.na(df$StDev_a)) #400
...
#Impute errors for missing SD - for cmin and cf separately and for each parameter separately (SOC, MAOC, POC)
...
hist(ratio_cmin) #looks identical - GOOD
...
###
#current dataframess = df and df.r 
#calculate SD from SE= SD/ sqrt(n)  => SE*sqrt(n)

#if the value is Na, replace with SD calculated from SE
SD.to.replace<-which(is.na(df$SD_a)); length(SD.to.replace) #of 751 obs
length(which(is.na(df$SE_a)))#number of na's

df$StDev_a<-df$SD_a
df$StDev_a<-ifelse(is.na(df$StDev_a),  #if SD = NA
         df[SD.to.replace ,]$SE_a * sqrt(df[SD.to.replace, ]$N_a), #replace with SE * sqrt of n
       df$StDev_a)
#remeasure the # of studies missing SD
length(which(is.na(df$StDev_a)))

#calculate the weight of each study based off of observation numbers (Hedges and Olkin 1985)
#According to Handbook, this is a good method if study variances are not reported
#but, if we impute SD, then can use the radom effects approach (handbook ch13
#HOwever, Handbook p 205 notes that if most of the studies do not report SD, it may be better to use a non-parametric approach)

###original code 
  #df$w_cf<-(df$N_a*df$N_cf)/(df$N_a+df$N_cf) #not weights
  #df$w_cmin<-(df$N_a*df$N_cmin)/(df$N_a+df$N_cmin) #not weights

  df$var.lnRR_cf<-(df$N_a*df$N_cf)/(df$N_a+df$N_cf)
  df$var.lnRR_cmin<-(df$N_a*df$N_cmin)/(df$N_a+df$N_cmin)

#handbook p. 205 recommends pairing the lnRR and ^ variance estimatation method with bootstrapping   
  
which(is.na(df$N_a))
which(is.na(df$N_cmin))
which(is.na(df$N_a))
which(is.na(df$N_cmin))

## Note that above calculations are actually for the variance of the log(RR) 
...
#create new df.r with ratios calculated for each obs
df.r<-cbind(df, ratio_cmin, percent.change.cmin, ratio_cf, percent.change.cf) #create df with response ratios
...
#subset by zero or fertilized controls
#remove rows with NA for response ratio - as these obvs cannot be analyzed
df.cf<-df.r[which(!is.na(df.r$ratio_cf)),]  
df.cmin<-df.r[which(!is.na(df.r$ratio_cmin)),]
...
#Create MAOC dataset
df.MAOC<-subset(df.r, param == "MAOC")
nrow(df.MAOC)

#Create POC dataset
df.POC<-subset(df.r, param == "POC")
nrow(df.POC)


#Create TOC dataset
df.TOC<-subset(df.r, param == "TOC") 
nrow(df.TOC)
