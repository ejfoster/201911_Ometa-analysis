...
## Previous analyses
All of the code saved here is for access to from previous stats
```{r, eval=FALSE}
###############################################################################################
...
```{r check for normality, eval=FALSE}
str(df.vine)
#colnames(df.vine)
...
```{r transformation of metals data, eval=FALSE}
#see "attempted" in 'extra' code below
```

```{r C linear regression models / ANOVA, eval=FALSE}
...
```{r N linear regression models / ANOVA, eval=FALSE}
 #set up liner mixed effects model, with Block as a fixed effect
...
```{r Metals - check for correlations between response variables, eval=FALSE}
...
```{r METALS check RCRA metals/standards, eval=FALSE}
...
```{r Anova - seperate for each metals of interest, eval=FALSE}
