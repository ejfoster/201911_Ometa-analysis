...
df.c<-subset(df2, param_group == "C") #subset by carbon parameters only, obs=2165
...
#estimate error
df.est<- df.c %>% 
  group_by(ACC, param, depth.end) %>% 
  mutate(min.var = var(df.c$ratio_min), fert.var = var(df.c$ratio_fert))
df.est

#how many studies have actual measurement of error? 28% of controls 36% amend
nrow(df.c)
se.error<-(1-length(which(is.na(df.c$SE_cf)))/nrow(df.c))*100
sd.error<-(1-length(which(is.na(df.c$SD_cf)))/nrow(df.c))*100
control.error<-se.error+sd.error
se.am.error<-(1-length(which(is.na(df.c$SE_a)))/nrow(df.c))*100
sd.am.error<-(1-length(which(is.na(df.c$SD_a)))/nrow(df.c))*100
error.amend<-se.am.error+sd.am.error; error.amend
```

```{r, calculate }
res.rma <- rma(stack.loss, vi, mods =  ~ Air.Flow + Water.Temp + Acid.Conc., data=df)
