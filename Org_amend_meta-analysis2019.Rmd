...
mixed model for meta-analysis with maximum likelihood
response = carbon stock 
...
How does amendment stabilizaiton of SC change with depth?
...
df1 %>% dplyr::select(X3,ACC, crop_group, crop_system, depth.start ,  depth.end ,  param ,  param_group ,  units , amend ,  irrigation ,  tillage_type , tillage_depth, till_depth_group, country , elevation ,  lat_group , lat.dec.degree, pH,  MAT ,  MAP ,  duration_group,  duration_even ,  clay_group ,  pH_group ,  SOC_g.kg, SOC_int_group,weight.dry,  C.rate_Mg.ha, C.N_rate, N.amend_g.kg, BD, X_cf, SE_cf, SD_cf, N_cf, X_a, SE_a, SD_a, N_a, X_cmin, SE_cmin, SD_cmin, N_cmin ) -> df 
...
```{r, 4. preliminary response ratios, eval=FALSE}
...
df.c$SD_cmin_stock.SE<-ifelse(df.c$units == "g.kg", 
...
df.c$SD_cf_stock.SE<-ifelse(df.c$units == "g.kg", 
...
```{r, impute error}
error.amend<-(1-length(which(is.na(df.c$StDev_a)))/nrow(df.c))*100; error.amend #33% have error reported
```
...
cf.rma1 <- rma(yi=df.c$ratio_fert, vi=, mods =  ~ amend + C.rate_Mg.ha + lat_group, data=df.c)
colnames(df.c)
?rma

mods<-cbind(a, b, c) ; mima (yi, vi, mods, method = "REML")

rma1<-rma(ratio_fert, vi=fert.var, data=df.est)
