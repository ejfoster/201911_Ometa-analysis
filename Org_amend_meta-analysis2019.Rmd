...
QT.comb<-c(mean(QT.cf, QT.zf))
 #chisq.test
 1-pchisq(q=QT.comb, df=1)
...
summary(all.rma.ar.nowt) # intercept (ACC) is highly significant
...
summary(all.rma.ar.null) # intercept (ACC?) is highly significant; AIC = 826
...
all.rma.ar.cat <- rma.mv(data=df.c, ratio_all ~ crop_group+irrigation+crop_system+lat_group + clay_group+ pH_group+ SOC_int_group+duration_even+C.rate_group+amend_N_kg.ha_group+amend_group, random= ~depth|ACC, method="REML", V=SD_all, W= w_all, struct="AR") #AIC 60
summary(all.rma.ar.cat) # 


#amendment group
all.rma.ar.am <- rma.mv(data=df.c, ratio_all ~ amend_group, random= ~depth|ACC, method="REML", V=SD_all, W= w_all, struct="AR") #AIC 284
