...
#left outer join
 df.c$ratio_all<-ifelse(is.na(df.c$ratio_cf),  df.c$ratio_zf, df.c$ratio_cf)
  df.c$SD_all<-ifelse(is.na(df.c$ratio_cf), mydf.zf$SD_all, mydf.cf$SD_all) #imputed std deviations
  df.c$w_all<-ifelse(is.na(df.c$ratio_cf), mydf.zf$w_all, mydf.cf$w_all)
...
```{r, 10. check for correlation among parameters}
colnames(df) 
hist(df.c$depth.end) 
...
param.corr<-select(df, MAT, MAP, lat.dec.degree, elevation, pH, clay, SOC_g.kg,
                  C.rate_Mg.ha, C.N_rate, N.amend_g.kg, time.n.mos, Nfert_rate)
param.corr$MAT.MAP<-df$MAT/df$MAP

pear<-rcorr(as.matrix(param.corr), type = "pearson") ; pear
#CORRELATED: pH and MAT = .98 and MAT/MAP; elevation and time.n.mos (.97); elevation and MAT.MAP .76,  lat and Nfert.rate .333

#remove eleveation and MAT
```
...
#With level = 1, we stick to models with main effects only. This implies
################################################################################

rma.glmulti <- function(data, formula, random, vi, V ) #vi = weight, V= SD
   rma(data=data, formula, random, vi, V, method="ML", struct="AR") #important to use ML (instead of REML) estimation, since log-likelihoods (and hence information criteria) are not directly comparable for models with different fixed effects
